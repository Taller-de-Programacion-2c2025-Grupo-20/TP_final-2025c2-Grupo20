...
WORKDIR="/tmp/${GAME_NAME}_server_repo"
REPO_DIR="$WORKDIR/TP_final-2025c2-Grupo20"

# Flag para saber si el servidor ya fue instalado
SERVER_FLAG="$ETC_DIR/server_installed.flag"
...
echo -e "${BLUE}=== Instalador del SERVIDOR de $GAME_NAME ===${NC}"
sleep 1

# Si ya está instalado y el binario existe, solo correr el servidor
if [ -x "$BIN_DIR/taller_server" ] && [ -f "$SERVER_FLAG" ]; then
    echo -e "${GREEN}→ Servidor ya instalado. Saltando instalación.${NC}"

    read -p "Puerto para el servidor (por defecto 9090): " SERVER_PORT
    SERVER_PORT=${SERVER_PORT:-9090}

    echo -e "${GREEN}→ Lanzando servidor en el puerto $SERVER_PORT ...${NC}"
    "$BIN_DIR/taller_server" "$SERVER_PORT"
    exit 0
fi
...
# INSTALAR DEPENDENCIAS (solo lo necesario para server)
...
echo -e "${GREEN}✓ Compilación del servidor finalizada.${NC}"
...
# INSTALAR BINARIO DEL SERVER
echo -e "${GREEN}→ Instalando ejecutable del servidor...${NC}"
sudo cp taller_server "$BIN_DIR/taller_server"

# INSTALAR DATA DEL SERVER (mapas, YAML, etc.)
echo -e "${GREEN}→ Instalando data del servidor en $VAR_DIR/maps_data ...${NC}"
sudo cp -r "$REPO_DIR/server/data/"* "$VAR_DIR/maps_data"

# Crear flag de instalación
echo -e "${GREEN}→ Marcando instalación de servidor como completada...${NC}"
echo "installed_on=$(date)" | sudo tee "$SERVER_FLAG" >/dev/null

echo -e "${GREEN}============================================${NC}"
echo -e "${GREEN} Instalación completa del SERVIDOR $GAME_NAME${NC}"
echo -e "${GREEN}============================================${NC}"

# EJECUTAR SERVIDOR AL FINAL (primera vez)
read -p "Puerto para el servidor (por defecto 9090): " SERVER_PORT
SERVER_PORT=${SERVER_PORT:-9090}
