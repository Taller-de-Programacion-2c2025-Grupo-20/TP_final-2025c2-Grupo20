...
void ClientHandler::stop() {
    ...
    try {
        ...
        std::cerr << "El cliente ya estaba cerrado: " << e.what() << std::endl;
    }
    receiver.stop();
    sender.stop();
}
...
void ClientHandler::set_username(const std::string& name) { this->username = name; }

std::string ClientHandler::get_username() const { return this->username; }

void ClientHandler::set_car_id(uint8_t id) { this->car_id = id; }

uint8_t ClientHandler::get_car_id() const { return this->car_id; }
...
void ClientHandler::send_login_failed() { protocol.send_login_failed(); }
...
void ClientHandler::send_match_list(const MatchListDTO& list) { protocol.send_match_list(list); }

void ClientHandler::send_start_game() { protocol.send_start_game(); }
